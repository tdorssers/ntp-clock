 ...
 * - Added support for search string without a question mark to find_key_val()
 * - Added find_key_val_p() for keys stored in progmem space
 * - Optimized h2int() for size
 *
...
uint8_t find_key_val(char *str,char *strbuf, uint8_t maxlen,char *key)
{
	...
	while(*str &&  *str!=' ' && *str!='\r' && found==0){
		if (*str == *kp){
			...
			// support for search string without a question mark, by Tim Dorssers
			if (kp==key &&  ! ( *(str-1) == '?' || *(str-1) == '&' || sp == str ) ) goto NEXT;
			kp++;
			if (*kp == '\0'){
				str++;
				kp=key;
...
// enough storage for strbuf, maxlen is the size of strbuf.
// I.e the value it is declared with: strbuf[5]-> maxlen=5
uint8_t find_key_val_p(char *str,char *strbuf, uint8_t maxlen,const char *progmem_key)
{
	uint8_t found=0;
	uint8_t i=0;
	const char *kp;
	...
	while(*str &&  *str!=' ' && *str!='\r' && found==0){
		if (*str == pgm_read_byte(kp)){
			...
			// match on 'foobar' when only looking for 'bar', by andras tucsni
			// support for search string without a question mark, by Tim Dorssers
			if (kp==progmem_key &&  ! ( *(str-1) == '?' || *(str-1) == '&' || sp == str ) ) goto NEXT;
			kp++;
...
uint8_t parse_ip(uint8_t *ip_byte_str,const char *str)
{
	...
	if (i==3){ // must have read the first components of the IP
		strbuf[bufpos]='\0';
		ip_byte_str[i]=(atoi(strbuf)&0xff);
		return(0);
	}
