 ...
 *
 * The TCP implementation uses some size optimizations which are valid
 * only if all data can be sent in one single packet. This is however
 * not a big limitation for a micro controller as you will anyhow use
...
static uint8_t arpip_state=0; // 0 at power on, 1=req sent no answer yet, 2=have mac, 8=ready to accept an arp reply
...
// return 1 on successful processing of answer
...
#ifdef UDP_client
// -------------------- send a spontaneous UDP packet to a server 
...
uint8_t gratutious_arp(uint8_t *buf)
{
	...
	buf[ETH_TYPE_L_P] = ETHTYPE_ARP_L_V;
	// arp request and reply are the same except for
	// the opcode:
...
// not answer. A timer would be needed to recognize such a condition.
//
	...
	if ( buf[TCP_DST_PORT_H_P]==TCPCLIENT_SRC_PORT_H){
		...
		if (tcp_client_state==2){
			if ((buf[TCP_FLAGS_P] & TCP_FLAGS_SYN_V) && (buf[TCP_FLAGS_P] &TCP_FLAGS_ACK_V)){
				...
				// which will be handled above. In other words there is
